...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        # out을 x와 동일한 dtype/device로 생성
        out = x.new_zeros((N, Ff, H_out, W_out))
            ...
            for f in range(Ff):
                for i in range(H_out):
                    ...
                    for j in range(W_out):
                        ws = j * stride
                        # x의 일부분을 잘라서 w와 연산하는데 x의 범위를 넘어갈 경우 0으로 padding
            ...
            for j in range(C):
                for k in range(H_out):
                    ...
                    for l in range(W_out):
                        ws = l * stride
                        sub = x[i, j, hs:hs+pool_height, ws:ws+pool_width]
                        m = sub.max()
                        mask = (sub == m)

                        count = mask.sum() # 최대값이 몇 개였는지 셈
                        g = dout[i, j, k, l] / count # dout을 최대값의 개수만큼 나누어서 균등 분배 시킴 
                        
                        # dout을 균등 분배 해줌

                        # mask를 같은 dtype으로 변환할 때 .to() 대신 .type_as 사용
                        dx[i, j, hs:hs+pool_height, ws:ws+pool_width] += mask.type_as(x) * g
...
class BatchNorm(object):
    ...
    def backward_alt(dout, cache):
        ...
        if mode == 'train':
            # 간단식: dxhat, 합 두 개, 한 줄로 dx
            dxhat = dout * gamma                          # (N, D)
            sum_dxhat = torch.sum(dxhat, dim=0)           # (D,)
            sum_dxhat_xhat = torch.sum(dxhat * x_norm, dim=0)  # (D,)

            dx = (dxhat - sum_dxhat / N - x_norm * (sum_dxhat_xhat / N)) / sqrt_var

        elif mode == 'test':
            # 러닝 통계는 상수이므로 선형 스케일만 전달
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        out2, cache = BatchNorm.forward(x2, gamma, beta, bn_param)
        # (N*H*W, C) -> (N, H, W, C) -> (N, C, H, W)
        out = out2.view(N, H, W, C).permute(0, 3, 1, 2).contiguous()
    ...
    def backward(dout, cache):
        ...
        N, C, H, W = dout.shape
        # (N, C, H, W) -> (N, H, W, C) -> (N*H*W, C)
        dout2 = dout.permute(0, 2, 3, 1).contiguous().view(-1, C)
        # alt 또는 standard 둘 다 가능
        dx2, dgamma, dbeta = BatchNorm.backward_alt(dout2, cache)
        # (N*H*W, C) -> (N, H, W, C) -> (N, C, H, W)
        dx = dx2.view(N, H, W, C).permute(0, 3, 1, 2).contiguous()
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
