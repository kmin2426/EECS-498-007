...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        # out = XW + b
        # x (N,C,H,W) -> reshape to (N,C*H*W) == (N,D) 
        # w (C*H*W, M)
        # output: (N,M)
...
class ReLU(object):
    ...
    def forward(x):
        ...
        # ReLU forward pass
        out = torch.clamp(x,min=0) # out = torch.max(torch.zeros_like(x), x)
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        self.params['W1'] = torch.randn(D, H, dtype=dtype, device=device) * weight_scale # weight initialization
        self.params['b1'] = torch.zeros(H, dtype=dtype, device=device)
        self.params['W2'] = torch.randn(H, C, dtype=dtype, device=device) * weight_scale # weight initialization
        self.params['b2'] = torch.zeros(C, dtype=dtype, device=device)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        if not torch.is_tensor(loss):
            loss = scores.new_tensor(loss)

        # L2 Regularization (using multiplication 0.5)
        for i in range(1, self.num_layers + 1):
            W = self.params[f'W{i}']
            loss = loss + 0.5 * self.reg * torch.sum(W * W)

        # Backward
        dh, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dout, cache)
        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']  # reg * W
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            grads[f'W{i}'] += self.reg * self.params[f'W{i}']  # reg * W
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, optim_config={'learning_rate': 1e-1, }, device=device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
